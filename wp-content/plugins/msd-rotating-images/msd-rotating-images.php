	...
	class iThemesRotatingImages {
		...
		function admin_menu() {
			...
			if ( $found_series === false ) {
				add_menu_page( $this->_series . ' Getting Started', $this->_series, 'administrator', 'pluginbuddy-' . strtolower( $this->_series ), array(&$this, 'view_gettingstarted'), $this->_pluginURL.'/images/displaybuddy16.png' );
	...
	if ( isset( $_REQUEST['group_id'] ) && empty( $_REQUEST['cancelsave_group'] ) ) { // dustin
							...
							<tbody>
								...
								<?php foreach ( (array) $this->_options['image_ids'] as $id => $entry ) : ?>
									<?php
										...
										$thumb = iThemesFileUtility::resize_image( $entry['attachment_id'], $thumb_width, $thumb_height, false );
		...
		<?php if ( ! isset( $this->_errors ) || isset( $_REQUEST['image_id'] ) || ( isset( $_REQUEST['action'] ) && ( 'save_image' === $_REQUEST['action'] ) ) ) : ?>
			<div class="wrap">
				...
				<p>Images not matching the exact size will be resized to fit upon display.</p>
				...
				<?php
					...
					if ( ! empty( $this->_options['attachment_id'] ) ) {
						...
						$image = iThemesFileUtility::resize_image( $this->_options['attachment_id'], $thumb_width, $thumb_height, false );
					}
