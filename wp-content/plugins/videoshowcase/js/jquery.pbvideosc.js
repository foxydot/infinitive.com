	...
	$.fn.pbvideosc = function(pbv_settings) {
		...
		*/
		$.pbvideosc.open = function() {
			...
			if(document.all && !window.XMLHttpRequest) $('select').css('visibility','hidden'); // To fix the bug with IE select boxes
		...
		*/
		$.pbvideosc.close = function(){
			...
			$pbv_overlay.fadeOut(settings.animation_speed, function(){
				settings = pbv_settings;
				if(document.all && !window.XMLHttpRequest) $('select').css('visibility','visible'); // To fix the bug with IE select boxes
		...
		*/
		function _getDimensions(width,height){
			...
			if(document.all && window.XMLHttpRequest && !document.querySelector) detailsHeight+=8;
			$pbv_details.remove();
