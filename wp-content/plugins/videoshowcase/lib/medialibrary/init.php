...
if ( ! class_exists( 'IT_Media_Library' ) ) {
	class IT_Media_Library {
		var $_version = '1.1.0'; // Change this in lib/medialibrary/load.php as well.
		...
		function filter_media_upload_library() {
			if ( !empty( $_POST['save'] ) ) {
				...
				echo '</pre>';
				*/
				echo 'iThemes sending data to parent window... Please wait...';
		...
		function get_add_link( $args ) {
			...
			$args['rel']    = empty( $args['rel'] ) ? '' : ' rel="' . $args['rel'] . '"';

			if ( $this->_pre_wp_3_5_compatibility )
				return '<a ' . $args['id'] . 'href="' . $this->get_link() . '" class="' . esc_attr( $args['classes'] ) . ' thickbox" ' . $args['styles'] . $args['rel'] . '>' . $args['text'] . '</a>';
			else
				return '<a ' . $args['id'] . 'href="#" class="' . esc_attr( $args['classes'] ) . ' add-new-image" ' . $args['styles'] . $args['rel'] . '>' . $args['text'] . '</a>';
		}
