...
class Ithemes_Updater_Server {
	...
	public static function request( $action, $query = array(), $data = array() ) {
		...
		if ( ! empty( $body['error'] ) )
			return new WP_Error( $body['error']['type'], sprintf( __( 'An error occurred when communicating with the iThemes update server: %s (%s)', 'it-l10n-videoshowcase' ), $body['error']['message'], $body['error']['code'] ) );
