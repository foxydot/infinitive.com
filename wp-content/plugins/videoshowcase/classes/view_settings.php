...
<div class="wrap">
	...
	if ( isset( $_GET['video_id'] ) ) {
		...
		echo '<table class="form-table">';
			...
			echo '</tr>';
			echo '<tr>';
				echo '<th><label for="video_img">Current Image</label></th><td>';
				if ( ! empty( $video['vimage'] ) ) {
					$imagedata = wp_get_attachment_image_src( $video['vimage'], 'thumbnail' );
					echo '<img id="current_image" src="' . $imagedata['0'] . '" alt="' . $video['vtitle'] . '" />';
				} else {
					_e( 'No Image', 'it-l10n-videoshowcase' );
				}
			echo '</td></tr>';
			echo '<tr>';
