...
class ShowContainer extends FancyObjectContainer{
	...
	function ShowContainer(){
		...
		$this->default_sort_dir = array('asc','asc','asc','asc','desc','asc');
	}
    ...
    function setShowLineup($ShowID, $ArtistIDs = array(),$hostArtistID = ""){
		...
		if (empty($ArtistIDs)){
            ...
            foreach ($ArtistIDs as $ArtistID){
                    if (PEAR::isError($result = $this->addShowArtist($ShowID,$ArtistID,$ArtistID == $hostArtistID))){
                            return $result;
                    }
            }
    }
    
	function addShowArtist($show_id,$Artist,$ArtistIsHost){
	...
	function getShow($show_id,$ArtistSort = "ArtistFullName",$ArtistSortDir = "asc"){
	...
	function getAllShows($ShowYear = "",$ScheduleUID = "",$ShowSortField = array('ShowDay','ShowStage','ShowStartTime','ShowID'),$ShowSortDir = array('asc','asc','asc','asc'),$ArtistSortField = 'ArtistFullName',$ArtistSortDir = 'asc',$PutHostFirst = true){
	...
	function getAllShowsWhere($whereClause = "",$ShowSortField = array('ShowDay','ShowStage','ShowStartTime','ShowID'),$ShowSortDir = array('asc','asc','asc','asc'),$ArtistSortField = 'ArtistFullName',$ArtistSortDir = 'asc',$PutHostFirst = true){
	...
	function getLineupsForSimpleShows($AllShows,$ShowSortField = array('ShowDay','ShowStage','ShowStartTime','ShowID'),$ShowSortDir = array('asc','asc','asc','asc'),$ArtistSortField = 'ArtistFullName',$ArtistSortDir = 'asc',$PutHostFirst = true){
		...
		for ($s = 0; $s < count($AllShows); $s += $BatchSize){
			...
			foreach ($ArtistSortField as $index => $field){
				$sort_field[] = $this->getTableName('Artist').".".$field;
		...
		function prettifyShow(&$Show){
	        ...
	        // Set Some variables that will be handy
				...
				if ($Show->getParameter('ShowEndTime') != ""){
				        $Show->setParameter('ShowPrettyEndTime',$this->Festival->getPrettyTime($Show->getParameter('ShowEndTime')));
				}
