<?php
    ...
    class FestivalApp__UserFunctions extends UserFunction{
        ...
        function FestivalApp__UserPaint($parms,$package,&$smarty){
            ...
            $ArtistURL = user__getArticleURL($parms['artist_article'],$Year).'artist=';
			...
			case 'lineup_list':
                ...
                if (!is_array($Lineup)){
					...
					if ($parms['subject'] == 'lineup' and $_GET['artist'] != "" and array_key_exists($_GET['artist'],$Lineup)){
						// Let's display the Artist Detail
						$Artist = $Lineup[$_GET['artist']];
            ...
            case 'all_festival_artists':
                ...
                foreach ($AllFestivalArtists as $FestivalArtist){
                    ...
                    if ($tmpFestival->getParameter('FestivalLineupIsPublished')){
                        $return.= "<li>";
                        $return.= "<a href='".user__getArticleURL($ArtistDetailPage,$FestivalArtist->getParameter('FestivalYear'))."&artist=".$FestivalArtist->getParameter('ArtistID')."'>".$FestivalArtist->getParameter('ArtistFullName')."</a>\n";
                        $return.= " (<a href='".user__getArticleURL($ArtistDetailPage,$FestivalArtist->getParameter('FestivalYear'))."&artist=".$FestivalArtist->getParameter('ArtistID')."'>".$FestivalArtist->getParameter('FestivalYear')."</a>";
                        $LIOpened = true;
                        $sep = ",";
                    }
                    if (is_array($FestivalArtist->getParameter('AllFestivalYears'))){
                        if (!$LIOpened){
                            ...
                            if ($tmpFestival->getParameter('FestivalLineupIsPublished')){
                                $return.= "<li>";
                                $return.= "<a href='".user__getArticleURL($ArtistDetailPage,current($FestivalArtist->getParameter('AllFestivalYears')))."&artist=".$FestivalArtist->getParameter('ArtistID')."'>".$FestivalArtist->getParameter('ArtistFullName')."</a>";
                                $return.= " (";
                                $LIOpened = true;
                            }
                        }
                        foreach ($FestivalArtist->getParameter('AllFestivalYears') as $FestivalYear){
                            ...
                            if ($tmpFestival->getParameter('FestivalLineupIsPublished')){
                                if ($FestivalYear != $FestivalArtist->getParameter('FestivalYear')){
                                    if (!$LIOpened){
                                        $return.= "<li>";
                                        $return.= "<a href='".user__getArticleURL($ArtistDetailPage,$FestivalYear)."&artist=".$FestivalArtist->getParameter('ArtistID')."'>".$FestivalArtist->getParameter('ArtistFullName')."</a>";
                                        $return.= " (";
                                        $LIOpened = true;
                                        $sep = "";
                                    }
                                    $return.= $sep."<a href='".user__getArticleURL($ArtistDetailPage,$FestivalYear)."&artist=".$FestivalArtist->getParameter('ArtistID')."'>".$FestivalYear."</a>";
                                    $sep = ",";
                                }
                            }
                        }
					...
					******************************************/
					...
					foreach ($FestivalArtists as $key => $Artist){
							...
							*/
							...
							$query = "SELECT Shows.ShowID FROM ".$ShowContainer->getTableName('Show')." Shows, ".$ShowContainer->getTableName()." Artist, ".$ShowContainer->getTableName('Schedule')." Schedule WHERE Shows.ShowID = Artist.ShowID AND Schedule.ScheduleUID = Shows.ShowScheduleUID AND Artist.ArtistID = $a AND Shows.ShowYear = '".addslashes($Year)."' AND Schedule.ScheduleIsPublished = 1 ORDER BY Shows.ShowDay ASC, Shows.ShowStartTime ASC";
