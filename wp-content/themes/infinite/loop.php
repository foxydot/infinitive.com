...
<?php elseif(is_page() || 'msd_casestudy' == get_post_type()) : ?>
		<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<?php else : ?>
		<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			<<?php print is_front_page()?'h2':'h1'; ?> class="entry-title">
				<?php if(!is_single()){ ?><a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'infinite' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php } ?>
				<?php the_title(); ?>
				<?php if(!is_single()){ ?></a><?php } ?>
			</<?php print is_front_page()?'h2':'h1'; ?>>
