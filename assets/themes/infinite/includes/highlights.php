...
function infinitive_case_studies($args = array(),$single=FALSE){
    ...
    usort($all_posts,'infinitive_posts_sort');
    if($single){
        return $all_posts[0];
    }else{
        return $all_posts;
...
function infinitive_grid_system($page_slug){
	switch($page_slug){
        ...
        case 'all-case-studies':
            $terms = get_terms('msd_practice-area', array('hide_empty' => FALSE));
            foreach($terms AS $term){
                $args = array(
                    'msd_practice-area' => $term->slug,
                );
                $my_posts = infinitive_case_studies($args,FALSE);
                $my_post = $my_posts[0];
                $more = count($my_posts)>1?'<div class="more"><a href="/infinitive-case-studies/'.$term->slug.'">View More '.$term->name.' &rarr;</a></div>':'';
                $grid .= '<div class="case_study_grid '.$term->slug.'">
                <div class="image"></div>
                <div class="grid_content">
                    <div class="most-recent"><h3><a href="'.msdlab_permalink($my_post->guid).'">'.$my_post->post_title.'</a></h3> <a href="'.msdlab_permalink($my_post->guid).'" class="read-more">Continue Reading &rarr;</a></div>
                    '.$more.'
                </div>
                <div class="clear"></div>
                </div>';
